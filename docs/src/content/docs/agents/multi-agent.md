...
snapshot(action="save", path="nb.ipynb", name="before-parallel-work")

# Agent A: lock and work on the import section (cells 0-3)
cell_locks(action="acquire", path="nb.ipynb", cell_ids=["cell0", "cell1", "cell2", "cell3"], owner="agent-a")

# Agent B: lock and work on the visualization section (cells 8-11)
cell_locks(action="acquire", path="nb.ipynb", cell_ids=["cell8", "cell9", "cell10", "cell11"], owner="agent-b")

# Both agents work simultaneously — locks prevent accidental overlap
# When done, release locks and verify with snapshot(action="diff")
```
...
- **Renew locks if your work takes longer** — calling `cell_locks(action="acquire")` again on cells you own resets the TTL
...
- **`copy_cells(delete_source=true)`** — transfer cells between notebooks (removed from source)
...
- **Bad edit** — `snapshot(action="restore", path, name="before-parallel-work")` rolls back the entire notebook (auto-saves a backup first)
...
| Tool | Purpose |
|------|---------|
| [`cell_locks`](../../tools/collaboration/#cell_locks) | Acquire, release, or list advisory cell locks |
...
| [`snapshot`](../../tools/collaboration/#snapshot) | Save, restore, list, or diff named checkpoints |
| [`copy_cells`](../../tools/editing/#copy_cells) | Copy/move cells between notebooks |
