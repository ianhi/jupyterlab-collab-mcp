...
An MCP server that lets AI assistants read, edit, and execute Jupyter notebooks with real-time collaboration. Designed in collaboration with agents actively using it and giving feedback ([For Agents, by Agents](./dogfooding/)). Tools are built to preserve agent context — a variable inspector that provides structured metadata without verbose output, outlines, diffs, incremental change polling, and image limits let agents stay aware of what's happening without burning through their context window. Changes appear instantly in your browser — no reloading. The server sees where your cursor is and won't touch the cell you're editing.
...
2. **Start JupyterLab** with [jupyter-collaboration](https://github.com/jupyterlab/jupyter-collaboration) enabled:
   ...
   See the [`jlabx` docs](https://ianhuntisaak.com/jlabx/) for configuration and environment detection.
...
- **39 MCP tools** — read, edit, execute, search, diff, tag, lock, snapshot, and more
- **AI-friendly variable inspector** — structured kernel introspection with smart truncation: DataFrames show columns/dtypes, arrays show shapes, dicts show keys — all without verbose output that wastes context
...
- **Context-aware** — agents stay informed without blowing their context window: outlines, diffs, incremental change polling, image limits, cell-type filtering, and output-free reads give situational awareness at minimal token cost
...
- _"Read the notebook analysis.ipynb"_ — [`get_notebook_content`](./tools/reading/)
- _"Run cell 3"_ — [`execute_cell`](./tools/execution/)
- _"Add a cell that imports pandas"_ — [`insert_cell`](./tools/editing/)
- _"Search for any cells with errors"_ — [`search_notebook`](./tools/reading/#search_notebook)

See the [Tool Reference](./tools/) for the full list of 39 tools.
