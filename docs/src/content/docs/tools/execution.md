...
Execute one or more cells in the notebook's kernel. Supports single-cell, range, and cell ID modes. Outputs appear in JupyterLab and are returned here.
...
| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `end_index` | number | No | — | Last cell index (inclusive). Triggers range mode when combined with `index` or `start_index` |
| `start_index` | number | No | `0` | First cell index for range execution |
| `cell_ids` | string[] | No | — | Cell IDs to execute in order (alternative to index range) |
...
# Run entire notebook (range mode)
execute_cell(path="nb.ipynb", start_index=0)

# Run cells 0-5
execute_cell(path="nb.ipynb", start_index=0, end_index=5)

# Run specific cells by ID
execute_cell(path="nb.ipynb", cell_ids=["a3f8c2d1", "b7e4f9a2", "c1d2e3f4"])
```
...
- Range mode continues on error and returns per-cell status
...
Post-process cached execution output. Use after `execute_cell` or `execute_code` to filter or paginate large outputs without re-executing.
